...
from .augment import generate_permutation_keys, permute_data
from .augment import reverse_permute_data
...
def patch_wise_prediction(model, data, overlap=0, batch_size=1, permute=False):
    ...
    return reconstruct_from_patches(predictions, patch_indices=indices,
...
def predict_from_data_file_and_get_image(model, open_data_file, index):
    return predict_and_get_image(model, open_data_file.root.data[index],
                                 open_data_file.root.affine)


def predict_from_data_file_and_write_image(model, open_data_file, index,
                                           out_file):
...
def run_validation_case(data_index, output_dir, model, data_file,
                        training_modalities, output_label_map=False,
                        threshold=0.5, labels=None, overlap=16, permute=False):
    """
    Runs a test case and writes predicted images to file.
    :param data_index: Index from of the list of test cases to get an image
        prediction from.
    :param output_dir: Where to write prediction images.
    :param output_label_map: If True, will write out a single image with one
        or more labels. Otherwise outputs
    the (sigmoid) prediction values from the model.
    :param threshold: If output_label_map is set to True, this threshold
        defines the value above which is considered a positive result and will
        be assigned a label.
    :param labels:
    ...
    if isinstance(prediction_image, list):
        ...
        prediction_image.to_filename(os.path.join(output_dir,
                                                  "prediction.nii.gz"))


def run_validation_cases(validation_keys_file, model_file, training_modalities,
                         labels, hdf5_file,
                         output_label_map=False, output_dir=".", threshold=0.5,
                         overlap=16, permute=False):
    ...
    for index in validation_indices:
        ...
        run_validation_case(data_index=index, output_dir=case_directory,
                            model=model, data_file=data_file,
...
def predict(model, data, permute=False):
    if permute:
        ...
        for batch_index in range(data.shape[0]):
            predictions.append(predict_with_permutations(model,
                                                         data[batch_index]))
...
def predict_with_permutations(model, data):
    ...
    for permutation_key in generate_permutation_keys():
        ...
        predictions.append(reverse_permute_data(model.predict(temp_data)[0],
                                                permutation_key))
